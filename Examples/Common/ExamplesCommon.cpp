...
//-----------------------------------------------------------------------------
Ogre::CompositorWorkspace *ColibriGuiGraphicsSystem::setupCompositor()
{
...
//-----------------------------------------------------------------------------
void ColibriGuiGraphicsSystem::registerHlms()
{
	Ogre::ConfigFile cf;
...
//-----------------------------------------------------------------------------
void ColibriGuiGraphicsSystem::setupResources( void )
{
	struct ShaperSettings
	{
		...
		ShaperSettings(
			const char *_locale, const char *_fullpath, hb_script_t _script, bool _useKerning = false,
			Colibri::HorizReadingDir::HorizReadingDir _horizReadingDir = Colibri::HorizReadingDir::LTR,
	...
	shaperManager->setDefaultShaper( defaultFont + 1u, shaperSettings[defaultFont].horizReadingDir,
	...
	if( defaultFont == 1 )
		m_colibriManager->setSwapRTLControls( true );

	m_colibriCompoProvider = OGRE_NEW Ogre::CompositorPassColibriGuiProvider( m_colibriManager );
	Ogre::CompositorManager2 *compositorManager = mRoot->getCompositorManager2();
	compositorManager->setCompositorPassProvider( m_colibriCompoProvider );
...
//-----------------------------------------------------------------------------
ColibriGuiGraphicsSystem::ColibriGuiGraphicsSystem( GameState *gameState ) :
	GraphicsSystem( gameState ),
	m_colibriManager( 0 ),
	m_colibriCompoProvider( 0 )
{
#if OGRE_PLATFORM == OGRE_PLATFORM_APPLE
...
#	if defined( _UNICODE ) || defined( UNICODE )
		...
		WideCharToMultiByte( CP_OEMCP, 0, path, (int)wcslen( path ), &mWriteAccessFolder[0], size_needed,
							 NULL, NULL );
#	else
...
//-----------------------------------------------------------------------------
void ColibriGuiGraphicsSystem::deinitialize()
{
	if( m_colibriCompoProvider )
	{
		Ogre::CompositorManager2 *compositorManager = mRoot->getCompositorManager2();
		compositorManager->setCompositorPassProvider( 0 );
		delete m_colibriCompoProvider;
		m_colibriCompoProvider = 0;
	}
