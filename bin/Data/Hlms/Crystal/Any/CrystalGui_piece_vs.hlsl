...
@piece( custom_vs_preExecution )
	@property( !crystal_text )
		uint crystalDrawId = input.drawId + ((uint(gl_VertexID) - worldMaterialIdx[input.drawId].w) / 54u);
	@end @property( crystal_text )
		uint crystalDrawId = input.drawId + ((uint(gl_VertexID) - worldMaterialIdx[input.drawId].w) / 6u);
	@end
	#undef finalDrawId
	...
	@property( crystal_text )
		uint vertId = (uint(gl_VertexID) - worldMaterialIdx[input.drawId].w) % 6u;
